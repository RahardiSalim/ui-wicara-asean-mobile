...
WorkspaceSession workspaceFromJson(Map<String, dynamic> json) {
  ...
  final learningContextJson = _map(json['learning_context']);
  return WorkspaceSession(
    ...
    learningContext: _workspaceLearningContextFromJson(learningContextJson),
    phaseEvidence: _phaseEvidenceFromJson(
  ...
  return WorkspaceAppendResult(
    ...
    tutorResponse: tutorResponse is Map<String, dynamic>
        ? WorkspaceTutorResponse(
            ...
            nextActions: _stringList(
              tutorResponse['next_actions'] ??
                  tutorResponse['suggested_actions'],
            ),
            ...
            correctness: _string(tutorResponse['correctness']).isEmpty
                ? 'unknown'
                : _string(tutorResponse['correctness']),
            misconceptionStatus:
                _string(tutorResponse['misconception_status']).isEmpty
                ? 'none'
                : _string(tutorResponse['misconception_status']),
            confidence: _doubleOrNull(tutorResponse['confidence']) ?? 0,
            evaluationOutcome: _nullableString(
              tutorResponse['evaluation_outcome'],
            ),
            scaffoldLevel: _int(tutorResponse['scaffold_level']),
            evidenceRequest:
                tutorResponse['evidence_request'] is Map<String, dynamic>
                ? tutorResponse['evidence_request'] as Map<String, dynamic>
                : null,
            explanationCard:
...
WorkspaceLearningContext _workspaceLearningContextFromJson(
  Map<String, dynamic> json,
) {
  final originalTarget = _map(json['original_target']);
  final currentModule = _map(json['current_module']);
  final diagnosis = _map(json['diagnosis']);
  final diagnosisEvidence = json['diagnosis_evidence'] is Map<String, dynamic>
      ? json['diagnosis_evidence'] as Map<String, dynamic>
      : _map(diagnosis['evidence']);
  final alreadyUnderstood = json['already_understood'];

  return WorkspaceLearningContext(
    originalTargetConceptId: _nullableString(
      originalTarget['concept_id'] ?? json['original_target_concept_id'],
    ),
    originalTargetConceptCode: _nullableString(
      originalTarget['concept_code'] ?? json['original_target_concept_code'],
    ),
    originalTargetLabel: _string(
      originalTarget['title'] ??
          originalTarget['label'] ??
          json['original_target_label'],
    ),
    currentModuleConceptId: _nullableString(
      currentModule['concept_id'] ?? json['current_module_concept_id'],
    ),
    currentModuleConceptCode: _nullableString(
      currentModule['concept_code'] ?? json['current_module_concept_code'],
    ),
    currentModuleLabel: _string(
      currentModule['title'] ??
          currentModule['label'] ??
          json['current_module_label'],
    ),
    moduleRole: _string(currentModule['role'] ?? json['module_role']),
    diagnosisReason: _string(diagnosis['reason'] ?? json['diagnosis_reason']),
    diagnosisEvidence: diagnosisEvidence,
    alreadyUnderstood: alreadyUnderstood is List
        ? alreadyUnderstood.whereType<Object>().toList(growable: false)
        : const [],
    route: _stringList(json['route']),
    returnsToOriginalTarget: _bool(json['returns_to_original_target']),
  );
}

Map<String, List<Map<String, dynamic>>> _phaseEvidenceFromJson(Object? value) {
  if (value is! Map<String, dynamic>) {
    return const <String, List<Map<String, dynamic>>>{};
  }
  return value.map((phase, evidence) {
    final entries = evidence is List
        ? evidence.whereType<Map<String, dynamic>>().toList(growable: false)
        : const <Map<String, dynamic>>[];
    return MapEntry(phase, entries);
  });
}

WorkspacePosttestTrigger _workspacePosttestTriggerFromJson(
  Map<String, dynamic> json,
) {
  return WorkspacePosttestTrigger(
    status: _string(json['status']),
    reason: _string(json['reason']),
    learningGoalId: _nullableString(json['learning_goal_id']),
    trackId: _nullableString(json['track_id']),
    moduleId: _nullableString(json['module_id']),
    conceptCode: _nullableString(json['concept_code']),
    conceptTitle: _string(json['concept_title']),
    posttestSessionId: _nullableString(json['posttest_session_id']),
